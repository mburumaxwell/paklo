...
|securityAdvisoriesFile|**_Optional_**. Path to a JSON file containing security advisories in the format specified by Dependabot. When provided, security vulnerabilities from this file will be fetched and processed.|
|includeCveInformation|**_Optional_**. Include security vulnerability identifiers (CVE, GHSA, etc.) in pull request descriptions. When set to `true`, CVE information will be added to the pull request description header for easier visibility. Defaults to `false`.|
